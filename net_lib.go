...
func (h *NetHttping) Httping(url string, useProxy bool, result chan<- int) {
	go func() {  // 为了避免阻塞Httping，所以在此处起一个
		...
		randT := rand.Uint64() % uint64( gSetting.Data.HttpRequestRandTimeOutMillisecond) // 避免阻塞做一个随机延迟
...
func buildHTTPClient(isProxy bool) *http.Client {
	...
	client := &http.Client{Transport: transport,Timeout: time.Second * time.Duration(gSetting.Data.HttpTimeOutSecond)}  // 暂定三秒，避免有些注定收不到的请求完全占用了多个线程而卡住了
	return client
}